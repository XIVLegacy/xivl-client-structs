...
def expect_others_quiet(
    name: str, ir, symbols_doc, structs_doc, overlay, expected_failure: str
) -> None:
...
def main() -> int:
    ...
    expect_build_error(
        ...
        lambda: build_ir.refuse_unsafe_ref(
            "../xivl-captures/docs/README.md", "overlay sourceRef"
        ),
        "live_parent_path",
    )
    check(
        "B4d citation resolution reads tracked content, not the working tree",
    ...
    expect_invariant_fails(
        "I1 schema rejects a value outside an enum", validate_ir.i1_ir_schema, ir
    )

    bad_overlay = copy.deepcopy(overlay)
    bad_overlay["alignment"]["BCS-S-0001"] = {
        "bytes": 0,
        "reason": "planted",
        "sourceRefs": [],
    }
    expect_invariant_fails(
        "I2 overlay schema rejects a zero alignment",
        validate_ir.i2_overlay_schema,
        bad_overlay,
    )

    ir = copy.deepcopy(baseline)
    ir["types"][0]["alignment"] = {"kind": "exact", "bytes": 4, "reason": "planted"}
    expect_invariant_fails(
        "I3 a value appearing in a declared-unknown dimension is caught",
        validate_ir.i3_deferred_dimensions,
        ir,
    )
    expect_others_quiet(
        "I3 plants nothing else",
        ir,
        symbols_doc,
        structs_doc,
        overlay,
    ...
    expect_invariant_fails(
        "I4 a renumbered BCS-Y identifier is caught",
        validate_ir.i4_identifier_preservation,
        ir,
        symbols_doc,
        structs_doc,
    )
    ...
    expect_invariant_fails(
        "I5 a promoted confidence tier is caught",
        validate_ir.i5_confidence_preservation,
        ir,
        symbols_doc,
        structs_doc,
    )
    ...
    expect_invariant_fails(
        "I1 schema catches a dropped reverify method", validate_ir.i1_ir_schema, ir
    )
    expect_invariant_fails(
        "I5 a dropped reverify method is caught",
        validate_ir.i5_confidence_preservation,
        ir,
        symbols_doc,
        structs_doc,
    ...
    expect_invariant_fails(
        "I7 a dropped source reference is caught",
        validate_ir.i7_citation_round_trip,
        ir,
        symbols_doc,
        structs_doc,
    )
    ...
    expect_invariant_fails(
        "I8 a rewritten raw address is caught",
        validate_ir.i8_raw_fidelity,
        ir,
        symbols_doc,
        structs_doc,
    )
    ...
    ir = copy.deepcopy(baseline)
    field = next(m for t in ir["types"] for m in t["members"] if m["kind"] == "field")
    field["type"] = "uint64_t"
    expect_invariant_fails(
        "I8 a rewritten field type is caught",
        validate_ir.i8_raw_fidelity,
        ir,
        symbols_doc,
        structs_doc,
    ...
    ir = copy.deepcopy(baseline)
    victim = next(
        t for t in ir["types"] if any(m["kind"] == "unknown-span" for m in t["members"])
    )
    victim["members"] = [m for m in victim["members"] if m["kind"] != "unknown-span"]
    expect_invariant_fails(
        "I9 a deleted unknown span breaks the byte arithmetic",
        validate_ir.i9_layout_arithmetic,
        ir,
    )
    ...
    span_owner = next(
        t["id"] for t in baseline["types"] if any(m is span for m in t["members"])
    )
    live_overlay = {
        "overlayVersion": "1.0",
        "gameVersion": "1.23b",
    ...
    check(
        "I10 a populated overlay reaches its two IR values cleanly", report.failed == 0
    )
    check(
        "I10 the overlay alignment actually lands on the type",
        populated["types"][0]["alignment"]
        == {"kind": "exact", "bytes": 4, "reason": "planted", "source": "overlay"},
    )

    ir = copy.deepcopy(populated)
    next(t for t in ir["types"] if t["id"] == baseline["types"][0]["id"])[
        "alignment"
    ...
    rel_sources = {
        role: json.loads((REPO / path).read_text(encoding="utf-8"))
        for role, path in build_ir.RELATIONSHIP_SOURCES.items()
    }

    planted = copy.deepcopy(rel_sources)
    planted["receiver-map"]["inboundReceivers"][0]["bcsRefs"]["class"][0]["bcsId"] = (
        "BCS-Y-9999"
    )
    expect_build_error(
        "B8 a relationship edge citing an uncataloged symbol is refused",
        lambda: build_ir.build(symbols_doc, structs_doc, overlay, planted),
        "BCS-Y-9999",
    )

    planted = copy.deepcopy(rel_sources)
    planted["operation-map"]["operationClasses"][0]["opcodes"][0]["direction"] = (
        "clientbound"
    )
    expect_build_error(
        "B9 an operation claiming a direction the model does not carry is refused",
        lambda: build_ir.build(symbols_doc, structs_doc, overlay, planted),
        "only serverbound is modeled",
    )

    planted = copy.deepcopy(rel_sources)
    planted["receiver-map"]["inboundReceivers"][0]["opcodes"][0]["opcodeHex"] = "0xABCD"
    expect_build_error(
    ...
    planted = copy.deepcopy(rel_sources)
    row = next(r for r in planted["c2s-skeleton"]["rows"] if r.get("confirmedBinding"))
    ...
    planted = copy.deepcopy(rel_sources)
    victim = next(
        b for b in planted["lua-bridge"]["bindings"].values() if b.get("opcodes")
    )
    ...
    expect_invariant_fails(
        "I11 a relationship citing an unknown symbol is caught",
        validate_ir.i11_relationship_symbols,
        ir,
        symbols_doc,
    )

    ir = copy.deepcopy(baseline)
    bound = next(o for o in ir["relationships"]["opcodes"] if "c2sBindings" in o)
    bound["symbols"] = [
        s
    ...
    opcode["receivers"] = [n for n in opcode["receivers"] if n != receiver["name"]]
    expect_invariant_fails(
        "I12 an edge the far side does not name back is caught",
        validate_ir.i12_relationship_closure,
        ir,
    )
    ...
    for record in ir["relationships"]["luaBindings"]:
        if record["luaName"] == name:
            record["opcodes"] = [o for o in record["opcodes"] if o != bound["id"]]
    expect_invariant_fails(
    ...
    expect_invariant_fails(
        "I13 an opcode int disagreeing with its hex is caught",
        validate_ir.i13_opcode_identity,
        ir,
    )
    ...
    expect_invariant_fails(
        "I13 a summary count drifting from the rows is caught",
        validate_ir.i13_opcode_identity,
        ir,
    )
    ...
    expect_invariant_fails(
        ...
        validate_ir.i14_payload_type_bindings_deferred,
        ir,
    )

    rebuilt = build_ir.build(symbols_doc, structs_doc, overlay)
    check(
        "D1 two builds from identical inputs are byte-identical",
        build_ir.serialize(rebuilt) == build_ir.serialize(baseline),
    )
    check(
        "D2 the committed ir_catalog.json is what the builder produces",
        build_ir.OUT_PATH.read_text(encoding="utf-8") == build_ir.serialize(baseline),
    )
