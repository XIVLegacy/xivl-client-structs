...
def validate(manifest: dict, structs_doc: dict, symbols_doc: dict) -> list[str]:
    ...
    if len(evidence.get("decompArchives", [])) != 4:
    ...
    if (map_fields.get("0x448", {}).get("name"), map_fields.get("0x57C", {}).get("size"), map_fields.get("0x5F0", {}).get("type"), map_fields.get("0x9E8", {}).get("type")) != (
        "registration_gate", "0x001", "ApplicationMainRaptureElementContainer *", "SqwtDataSqwtXmlDataMaker *"
    ):
    ...
    if len(owner.get("route", [])) != 6 or owner.get("clientWork", {}).get("recordStride") != "0x78":
    ...
    for required in ("canonical opcode identity", "native marker class", "source-keyed", "not allocation ownership", "not ownership", "coordinate", "not marker presentation"):
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(f"ERROR: {error}")
        return 1
    print("s2c 0x018D presentation: 49 invariants passed")
    return 0
