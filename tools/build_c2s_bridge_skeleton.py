...
that the inverse-direction agent can decompile to verify. The matching is
token-based on the anchor names; false positives are expected. The point
is to surface the high-confidence targets so the next agent doesn't start
from a blank slate.
"""
