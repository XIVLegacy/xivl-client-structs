...
def main() -> int:
    ...
    for label, path, value in [
        ...
        (
            "resolved producer id",
            ("deterministicProducers", 4, "encodedId"),
            "unresolved",
        ),
        (
            "resolved producer listing",
            ("deterministicProducers", 4, "notes"),
            "decompiler guess",
        ),
        (
            "dynamic selector domain",
            ("deterministicProducers", 12, "selector"),
            "caller-supplied",
        ),
        ("packet selector domain", ("deterministicProducers", 2, "selector"), "0x08"),
        (
            "bounded selector-0x08 ids",
            ("deterministicProducers", 3, "encodedId"),
            "caller-supplied",
        ),
        ...
        (
            "vtable owner",
            ("dynamicSelectorDomain", "vtableOwners", 1, "vtable"),
            "0x00FD5A78",
        ),
        (
            "vtable result",
            ("dynamicSelectorDomain", "vtableOwners", 2, "result"),
            "0x00",
        ),
        (
            "auto-id callers",
            ("dynamicSelectorDomain", "autoIdWrapper", "directCallers", 1),
            "0x006E3650",
        ),
        ("FormElement fixed id", ("selector1AClosure", "fixedId"), "0xC0000025"),
        (
            "FormElement lifecycle",
            ("selector1AClosure", "lifecycleVerdict"),
            "actor creation",
        ),
        ("FormElement cache", ("selector1AClosure", "cacheEffect"), "fixed cache"),
        (
            "producer boundary",
            ("otherSelectorProducer", "interfaceVerdict"),
            "connected",
        ),
        (
            "effect upper-id boundary",
            ("otherSelectorProducer", "upperIdBoundary"),
            "unresolved",
        ),
        ...
        (
            "evidence recipe",
            ("method", "commands", 4),
            "tools/ghidra/export-references.ps1 -Address bad -Output bad",
        ),
        ("producer listing", ("evidence", "producerInstructionListing"), "missing"),
        (
            "producer listing recipe",
            ("method", "commands", 5),
            "DumpFunctionListing.java",
        ),
        (
            "producer reference completion",
            ("evidence", "producerReferenceExports", 0, "completion"),
            "partial",
        ),
        ("ClientWork closure", ("clientWorkClosure", "allocationSize"), "0x834"),
        (
            "ClientWork consumer",
            ("clientWorkClosure", "consumers", 1, "clear"),
            "unresolved",
        ),
    ]:
    ...
    next(f for f in container["fields"] if f["offset"] == "0x004AC")["type"] = (
        "unknown[0x48]"
    )
    mutations.append(("container embedding", manifest, changed_structs, symbols))
    changed_symbols = copy.deepcopy(symbols)
    next(s for s in changed_symbols["symbols"] if s["id"] == "BCS-Y-2182")[
        "address"
    ] = "0x00533550"
    mutations.append(("factory symbol", manifest, structs, changed_symbols))
    changed_symbols = copy.deepcopy(symbols)
    next(s for s in changed_symbols["symbols"] if s["id"] == "BCS-Y-2197")[
        "address"
    ] = "0x005F5F80"
