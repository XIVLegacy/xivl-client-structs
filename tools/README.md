...
| Validate selector 0x0D ClientWorkElement | `python tools\validate_rapture_selector_0d.py` | Asset-free selector, identity, factory, and field-boundary checks |
| Test selector 0x0D validation | `python tools\test_rapture_selector_0d.py` | Eleven planted defects covering routing, identity, lifecycle, fields, and consumers |
...
- `validate_rapture_selector_0d.py`: focused cross-catalog invariants for the selector `0x0D` table entry, ClientWorkElement allocation/class identity, promoted factory and teardown symbols, container cache, ClientWorkStorage field boundaries, and both direct consumers. `test_rapture_selector_0d.py` plants eleven independent mutations and requires each to fail.
