...
Use this guide to regenerate catalog products, run repository gates, and invoke
research tools with explicit local inputs.

## Human entry points

These are the public commands a person invokes. Other scripts documented below
...
- `data\vendor\`: vendored fixture promotions from first-party source projects, each under its own subdir (`opcodes\`, `captures\`) with a `PROVENANCE.json` recording source repo, source path, and the sha256 of the copied bytes. Fixtures are byte-identical promotions, not regenerated. `tools\validate_vendor.py` re-hashes every file against its entry and `tools\refresh_vendor.py` restores or re-pins it from a named source checkout.
...
- `_symbols_io.py`: the single home for `manifests\symbols.json` I/O. `load_symbols()` always opens UTF-8 to avoid Windows cp1252 decoding; `next_bcsy_id()` allocates the next `BCS-Y-NNNN` from `max()` over the parsed ids (never array position, which is not globally sorted); `append_symbol()` appends and syncs `symbolCount`; `write_symbols()` writes the house style (indent 2, `ensure_ascii=False`, LF, trailing newline) via a temp file plus `os.replace()`, so an interrupted write cannot truncate the catalog. Writers must wrap the whole cycle in `symbols_transaction()`, which holds an exclusive lock file across the re-read and the write: without it two concurrent processes allocate the same `BCS-Y-NNNN` and the later write silently drops the earlier writer's entry. Importing tools add `sys.path.insert(0, str(Path(__file__).resolve().parent))` then `from _symbols_io import load_symbols`. New readers or writers of symbols.json should use it rather than re-rolling the read / allocate / write.
...
- `validate_vendor.py`: the drift gate, run by `validate-json.ps1`. Re-hashes every file under `data\vendor\` against its `PROVENANCE.json` entry and fails on a hash mismatch, a missing or undeclared file, a vendor subdir with no `PROVENANCE.json`, or an entry missing its source citation fields.
...
Run these before a refresh and after edits to research notes. All should report 0 findings in a clean repository.
...
- `validate_pcap_bridge.py`: explicit-path research command, non-gating (requires `scapy`). Requires `--captures-dir` (no workspace default); validates the pcap-grounded s2c bridge against the given capture directory. Validate-only by default; pass `--write` to regenerate the committed `manifests/pcap_validation.json` sidecar (whose numbers are referenced across downstream ledgers - regenerate deliberately, not as a side effect of validating).
...
- `hygiene_scan.py`: combined hygiene scan: wiki-link integrity + matrix-vs-symbols reconciliation + duplicate-address, sourceRef-form (symbols.json + structs.json), and embedded-manifest-address-drift checks. A4 validates citation form without leaving the checkout: `repository:path` citations and maintainer-record labels pass by shape, in-repo relative refs resolve on disk, and any live parent-dir path is a gated defect. The maintainer-only orphaned-manifest inventory runs only with `--phase-ledger PATH`; the default gate never reads the ignored island. Exits 1 on A1 cross-reference breaks, A4 live parent-dir refs, A5 broken wiki-links, and A6 embedded-address drift. A2/A3 and missing in-repo refs such as ungenerated `build/` or local Ghidra logs stay advisory. Both `audit_matrix.py` modes likewise exit 1 on structural findings.
