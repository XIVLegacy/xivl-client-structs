...
Read-only. Produces a structured report covering:
  ...
  A3: Optional orphaned-manifest scan against an explicitly supplied
      maintainer investigation ledger.
  A4: SourceRef path normalization inventory (symbols.json + structs.json).
  ...
  A5: Wiki-link integrity sweep (folds in the earlier manual pass) over
  ...
  A6: Embedded {bcsId, address} pairs in the derived snapshot manifests
...
def scan_a5_wikilinks(symbols, structs):
...
def scan_a6_embedded_addresses(symbols):
    ...
    to its id at generation time. The id is the join key (kept green by the
    four permanent audits); the embedded address is a convenience copy that
    ...
    for mf in sorted(MANIFESTS_DIR.glob("*.json")):
        ...
        for obj in _walk_json_objects(data):
            ...
            if canon is None:
                continue  # unresolved id is the four audits' concern, not this
...
def main(argv: list[str] | None = None) -> int:
    ...
    a4 = scan_a4_sourcerefs(symbols, structs)
    print("category counts:")
    for cat, count in sorted(a4["categories"].items()):
        print(f"  {cat}: {count}")
    print(f"\nabsolute-path examples ({len(a4['absolute_examples'])} total shown up to 20):")
    for ex in a4["absolute_examples"]:
    ...
    for ex in a4["backslash_examples"][:10]:
        print(f"  {ex['id']}: {ex['ref']}")
    print(f"\ncitation examples ({a4['categories'].get('citation', 0)} total shown up to 10):")
    for ex in a4["citation_examples"]:
        print(f"  {ex['id']}: {ex['ref']}")
    print(f"\nlive parent-dir paths (defect - gated): {len(a4['live_parent_paths'])}")
    for ex in a4["live_parent_paths"]:
        print(f"  {ex['id']}: {ex['ref']}")
    print(f"\nin-repo relative refs MISSING on disk (reported - not gated, "
          f"see exit-code note): {len(a4['missing_repo_relative'])}")
    for ex in a4["missing_repo_relative"][:10]:
        print(f"  {ex['id']}: {ex['ref']}")

    print("\n--- A5: wiki-link [[name]] integrity sweep ---\n")
    a5 = scan_a5_wikilinks(symbols, structs)
    print(f"valid target set size: {a5['valid_target_count']}")
    print(f"total [[link]] occurrences: {a5['total_occurrences']}")
    print(f"  pass: {a5['pass']}")
    skip_total = sum(a5["skip_by_category"].values())
    print(f"  skip (false positives): {skip_total}")
    for cat, count in sorted(a5["skip_by_category"].items()):
        print(f"    {cat}: {count}")
    print(f"  fail (real broken links): {len(a5['fail'])}")
    for f in a5["fail"]:
    ...
    print("\n--- A6: embedded manifest address vs symbols.json canonical ---\n")
    a6 = scan_a6_embedded_addresses(symbols)
    print(f"derived manifests scanned: {a6['manifests_scanned']}")
    print(f"embedded bcsId+address pairs checked: {a6['pairs_checked']}")
    print(f"pairs whose address drifts from symbols.json: {len(a6['drift'])} "
