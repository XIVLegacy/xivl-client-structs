...
def check_entry(directory: Path, entry: object, errors: list[str]) -> str | None:
    """Validate one PROVENANCE entry and return the declared file name, if any."""
    ...
    if actual != declared:
        errors.append(
            ...
            f"the vendored copy drifted from {entry.get('sourceRepo')}:{entry.get('sourcePath')} "
    ...
    for directory in sorted(p for p in VENDOR.iterdir() if p.is_dir()):
        ...
        for path in sorted(directory.iterdir()):
            ...
            if path.name not in declared:
                errors.append(
                    f"{path.relative_to(REPO)}: vendored file has no PROVENANCE entry; "
                    f"promote it with tools/refresh_vendor.py or delete it"
                )
