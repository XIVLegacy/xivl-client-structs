...
WRITE_CHAIN_RE = re.compile(
    r"^([A-Za-z][A-Za-z0-9_]*)\.(0x[0-9a-fA-F]+)\.(0x[0-9a-fA-F]+)$"
)
...
def promote(curated: dict) -> int:
    ...
    for (cls, outer, inner), writers in chain_writes.items():
        for (a_cls, lua), a in fa_by_key.items():
            ...
            for nr in nested:
                if (
                    nr.get("outer", "").lower() == outer
                    and nr.get("inner", "").lower() == inner
                ):
                    for w in writers:
                        append_promotion(
                            {
    ...
    for lua, rule in SEMANTIC_OVERLAP.items():
        ...
        if not a or a.get("status") != "ok":
            print(
                f"  semantic-overlap candidate {lua}: missing field-access entry",
                file=sys.stderr,
            )
            continue
        ...
        if not any(acc.lower() in callees for acc in SUBSTRUCT_ACCESSORS):
            print(
                f"  semantic-overlap candidate {lua}: FUN_006EEC00 not in chasedCallees",
                file=sys.stderr,
            )
            continue
        ...
        if not writers:
            print(
                f"  semantic-overlap candidate {lua}: write chain {chain_key} not present",
                file=sys.stderr,
            )
            continue
        for w in writers:
    ...
    for p in promotions:
        print(
            f"  {p['luaName']:35s} <- {p['writingOpcode']}  "
