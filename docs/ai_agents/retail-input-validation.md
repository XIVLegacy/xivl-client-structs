...
The normal asset-free repository checks remain the merge gate. The retail-input
lane is an additional evidence check for one bounded claim. It proves that a
clean Ghidra import of an exact retail executable reproduces a fixed sanitized
observation set; it does not prove runtime behavior or the complete semantic
interpretation of the actor-rebuild transaction.
...
reviewer gate. A repository administrator must not bypass the environment
protection rule.
...
The actor-rebuild claim predates this lane. A failed or missing run blocks
describing it as retail-input-CI reproduced and blocks expansion of the lane.
