...
[`ir-schema.md`](ir-schema.md) for how those entries join into the generated
IR. The [`manifests/README.md`](../manifests/README.md) documents the BCS-Y
(Receiver-name) versus BCS-S (Packet-name) naming boundary applied throughout.
...
The local player's auto-run flag is bit 3 (`0x08`) of the dword at `+0x04` of
the CharaActor motion controller
...
the separate walk/run move mode) - Scene-layer state, not Element-layer state -
read by `FUN_007A4610` (BCS-Y-1839) and written only by `FUN_007B04A0`
